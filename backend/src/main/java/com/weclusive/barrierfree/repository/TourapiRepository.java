...
//	@Query("select t from Tourapi t inner join TourapiImpairment ti on t.contentId = ti.contentId where delYn = ?1 and contentId = ?2")
	...
	// 시도,시군구로 검색한 결과
	public Page<Tourapi> findByDelYnAndSidoCodeAndSigunguCode(char delYn, String sidoCode, String sigunguCode,
			Pageable pageable);

	// 시도,컨텐츠 타입으로 검색한 결과
	public Page<Tourapi> findByDelYnAndSidoCodeAndTourapiContenttypeid(char delYn, String sidoCode,
			String contentTypeId, Pageable pageable);
	...
	public Page<Tourapi> findBySidoCodeAndImpariments(char delYn, String sidoCode, List<String> impairments,
			Pageable pageable);
	...
	public Page<Tourapi> findByTourapiContenttypeidAndImpariments(char delYn, String contentTypeId,
			List<String> impairments, Pageable pageable);
	...
	public Page<Tourapi> findBySidoCodeAndSigunguCodeAndImpairments(char delYn, String sidoCode, String sigunguCode,
			List<String> impairments, Pageable pageable);

	// 시도,컨텐츠타입,장애정보로 검색한 결과
	@Query("select t from Tourapi t where t.delYn=:delYn and t.sidoCode = :sidoCode and t.tourapiContenttypeid = :contentTypeId and t.contentId in (select ti.contentId from TourapiImpairment ti where ti.code in :impairments)")
	public Page<Tourapi> findBySidoCodeAndTourapiContenttypeidAndImpariments(char delYn, String sidoCode,
			String contentTypeId, List<String> impairments, Pageable pageable);

	// 시도,시군구,컨텐츠타입,장애정보로 검색한 결과
	@Query("select t from Tourapi t where t.delYn=:delYn and t.sidoCode = :sidoCode and t.sigunguCode = :sigunguCode and t.tourapiContenttypeid = :contentTypeId and t.contentId in (select ti.contentId from TourapiImpairment ti where ti.code in :impairments)")
	public Page<Tourapi> findBySidoCodeAndSigunguCodeAndTourapiContenttypeidAndImpariments(char delYn, String sidoCode,
			String sigunguCode, String contentTypeId, List<String> impairments, Pageable pageable);
...
//	@Query(value = "SELECT t FROM Tourapi t WHERE t.tourapi_contenttypeid = :contenttypeid AND t.tourapi_title LIKE ")
	...
	public Page<Tourapi> findByTourapiTitleContainingAndTourapiContenttypeidIn(String keyword, String[] contenttypeid,
			Pageable pageable);
